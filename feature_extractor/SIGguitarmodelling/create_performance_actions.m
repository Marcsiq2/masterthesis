function create_performance_actions(batch)
...
for i=1:numberOfFiles, %for each file (do not count . and ..
    if ~(strcmp(files(i,1).name(1),'.'))  &&~(strcmp(files(i,1).name(max(end-6,1):end),'p2s.mat'))%if to by pass . and .. DOS comands listed by dir as files
        ...
        fprintf(['Performing aligment betwen performance and score od %s\n'], files(i,1).name);
        
        load([path_file_s,'/',files(i,1).name,'/',files(i,1).name,'_p2s.mat'],'p2s','nmat1','nmat2');%read all anotated data of one song
        ...
        score_s = pa(score_s,emb,p2s,nmat1,nmat2);
...
save([path_scores(1:end-9),'noteDB/emb.mat'],'emb_all');
save([path_scores(1:end-9),'noteDB/score_descript.mat'],'score_all');
...
function score_s = pa(score_s,emb,p2s,nmat1,nmat2)
j=0;%counter for performance matrix

for i=1:length(nmat1)
    if strcmp(score_s.emb(i),'n')
        if score_s.emb_count(i)==0
            %j=j+1
            score_s.boff(i)=0;
            score_s.dur_r_t(i)=0;
            score_s.dur_r_n(i)=0;
            ...
            score_s.boff(i)=emb.boff(j+1);
            score_s.dur_r_t(i)=emb.odd_r(j+1);
            score_s.dur_r_n(i)=nmat2(j+1,2)/nmat1(i,2);
            score_s.vel_r(i)=emb.odd(j+1);
            j=j+1;
        end
    else
        in=j;
        score_s.boff(i)=emb.boff(in+1);
        j=j+score_s.emb_count(i);
        score_s.dur_r_t(i)=sum(nmat2(in+1:j,2))/gettempo(nmat2);
        score_s.dur_r_n(i)=sum(nmat2(in+1:j,2))/nmat1(i,2);
        score_s.vel_r(i)=mean(nmat2(in+1:j,5))/80;
        
    end
end
score_s.dur_r_n=score_s.dur_r_n';
score_s.dur_r_t=score_s.dur_r_t';
score_s.vel_r=score_s.vel_r';
score_s.boff=score_s.boff';
