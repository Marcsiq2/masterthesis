...
def write_midi_notes(MyMIDI, track, pitch_m, onset_b, dur_b, vel):
    for i in range(len(pitch_m)):
        ...
        d = dur_b[i]
        v = vel[i]
        MyMIDI.addNote(track, 0, p, t, d, v)
...
def adapthres(signal):
    ...
    ro = 0.001  #input('input ro:') ro --> threshold
    lmbda = 0.8  # input('input lambda:') Q filter
...
def f0_2_midi_sig(f0, tuning):
    ...
    f0_midi = 12 * safe_log2(f0/tuning) + 69
    return f0_midi
            ...
            if any([cond1,cond2]):
                ...
                if prev_int==-next_int:
                    ...
                    if prev_int==next_int:
                        ...
                        Q[int(on_off_idx[i]+1):int(on_off_idx[i]+whb)]=Q[int(on_off_idx[i])]
                        whf=np.floor(next_note_len/2)#half part that goes foward
                        Q[int(on_off_idx[i]+whb+1):int(on_off_idx[i]+whb+whf)]=Q[int(on_off_idx[i]+whf+whb+1)]
